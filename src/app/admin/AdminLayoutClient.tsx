...
export default function AdminLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  if (isLoginPage) {
    return (
      <div className="min-h-screen bg-[#0B0F14] flex flex-col text-white relative z-40">
        {children}
        <Toaster />
      </div>
    );
    ...
    <div className="min-h-screen flex bg-[#0B0F14] text-white relative z-40">
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#111827] border-b border-gray-800 flex items-center justify-between px-4 z-50">
        <h2 className="text-xl font-heading font-bold text-white">
          EcoVista<span className="text-[#D4AF37]">Admin</span>
        </h2>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                    isActive
                      ? "bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]"
      ...
      <main
        className={`flex-1 ${isFullscreenEditor ? "" : "md:ml-56 p-4 md:p-6"} ${isFullscreenEditor ? "" : "pt-20 md:pt-6"} bg-[#0B0F14] min-h-screen flex flex-col`}
      >
